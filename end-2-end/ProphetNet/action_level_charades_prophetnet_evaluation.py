...
#         self.ref_src = [[self.action_class[i] for i in action_ids] for action_ids in action_src_reference]
#         print(action_tgt_reference[0])
        self.ref = [[self.action_class[i] for i in action_ids] for action_ids in action_tgt_reference]
        #         print(action_src_candidate[0])
        self.cand = [tgt_actions[0][0].split(" ") for tgt_actions in action_tgt_candidate]
#         self.ref = [action_src + [self.action_class[i] for i in action_ids] for action_src, action_ids in zip(action_src_reference, action_tgt_reference)]
# #         print(action_src_candidate[0])
#         self.cand = [src_actions + tgt_actions[0][0].split(" ") for src_actions, tgt_actions in zip(action_src_candidate, action_tgt_candidate)]
